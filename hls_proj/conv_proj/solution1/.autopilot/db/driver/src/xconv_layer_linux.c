...
typedef struct {
    ...
    XConv_layer_uio_map maps[ MAX_UIO_MAPS ];
} XConv_layer_uio_info;

/***************** Variable Definitions **************************************/
static XConv_layer_uio_info uio_info;
...
static int uio_info_read_name(XConv_layer_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XConv_layer_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XConv_layer_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XConv_layer_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XConv_layer_Initialize(XConv_layer *InstancePtr, const char* InstanceName) {
	XConv_layer_uio_info *InfoPtr = &uio_info;
