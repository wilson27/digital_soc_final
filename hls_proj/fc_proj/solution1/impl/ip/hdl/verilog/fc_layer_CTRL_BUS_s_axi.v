...
`timescale 1ns/1ps
module fc_layer_CTRL_BUS_s_axi
#(parameter
    ...
    output wire [31:0]                   num_inputs,
    output wire [31:0]                   num_outputs,
...
localparam
    ...
    ADDR_BATCH_SIZE_DATA_0    = 6'h20,
    ADDR_BATCH_SIZE_CTRL      = 6'h24,
    ADDR_NUM_INPUTS_DATA_0    = 6'h28,
    ADDR_NUM_INPUTS_CTRL      = 6'h2c,
    ADDR_NUM_OUTPUTS_DATA_0   = 6'h30,
    ADDR_NUM_OUTPUTS_CTRL     = 6'h34,
    ADDR_ENABLE_RELU_DATA_0   = 6'h38,
    ADDR_ENABLE_RELU_CTRL     = 6'h3c,
    WRIDLE                    = 2'd0,
    WRDATA                    = 2'd1,
    WRRESP                    = 2'd2,
    WRRESET                   = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [31:0]                   int_num_inputs = 'b0;
    reg  [31:0]                   int_num_outputs = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_NUM_INPUTS_DATA_0: begin
                    rdata <= int_num_inputs[31:0];
                end
                ADDR_NUM_OUTPUTS_DATA_0: begin
                    rdata <= int_num_outputs[31:0];
                end
...
assign num_inputs    = int_num_inputs;
assign num_outputs   = int_num_outputs;
...
always @(posedge ACLK) begin
    if (ARESET)
        int_num_inputs[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_NUM_INPUTS_DATA_0)
            int_num_inputs[31:0] <= (WDATA[31:0] & wmask) | (int_num_inputs[31:0] & ~wmask);
    end
end

// int_num_outputs[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_num_outputs[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_NUM_OUTPUTS_DATA_0)
            int_num_outputs[31:0] <= (WDATA[31:0] & wmask) | (int_num_outputs[31:0] & ~wmask);
    end
end
