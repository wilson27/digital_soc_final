...
//  char* in_buffer = NULL;
//  char* allocated = NULL;
//  Size of input array with weight, bias and input image
...
//  in_buffer = allocated;
//  memcpy(in_buffer, (void*)mem, size*sizeof(float));     // copy from mem to in_buffer 
...
//  printf("Wrote inputs via DMA\n");
  ...
  write_int(map_base, target + XFC_LAYER_CTRL_BUS_ADDR_AP_CTRL, 0x1);
  do {
    sleep(1);
  ...
  while (!(read_int(map_base, target+ XFC_LAYER_CTRL_BUS_ADDR_AP_CTRL ) & 0xe));    
